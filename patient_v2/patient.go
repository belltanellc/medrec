...
func (t *PatientChaincode) Init(stub shim.ChaincodeStubInterface) ([]byte, error) {
	...
	myLogger.Info("[PatientChaincode] Init")
	if len(args) != 0 {
	...
	if len(assignerRole) == 0 {
		return nil, errors.New("Invalid assigner role. Is Empty.")
	}
...
func (t *PatientChaincode) assign(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
...
func (t *PatientChaincode) transfer(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	if len(args) != 2 {
		return nil, errors.New("Incorrect number of arguments. Expecting 2")
	}
